...
class Controller_Customer extends Controller_Core_Action
{
	...
	public function importDataAction()
	{
		try {
			...
			$upload->setPath('customer/csv')->setExtensions(['csv'])->upload('csv');
			$file = $upload->getFile();
			$rows = Ccc::getModel('Core_File_Csv')->setPath($upload->getPath())->setFileName($file['name'])->get();
			$customerModel = Ccc::getModel('customer');
			foreach ($rows as $row) {
				$uniqueColumns = ['email'=>$row['email']];
				$customerModel->getResource()->insertUpdateOnDuplicate($row,$uniqueColumns);
			}
		} catch (Exception $e) {
			$this->getMessage()->addMessage($e->getMessage(),Model_Core_Message::FAILURE);
		}
	...
	{	
		try {
			...
			header('Content-Disposition: attachment; var/customer/csv/customer.csv');
